...
P = rn1d_p(.5, 0.25, 0.5, 0.49, 0.05, 7.39, 100.) # con = 100

@unpack a, ep, b, gama, th, d, con = P



f(O,Di,Dm,P::rn1d_p) = Point3f0(
    -O*P.d + P.b*(P.con -Di -Dm) + 2*P.ep*(P.con -Di -Dm) - O^2*P.a*Di,
    -Di*P.gama + P.ep*(P.con -Di -Dm) + P.th*Dm + (-1/2)*O^2*P.a*Di,
    Di*P.gama - P.th*Dm)
...
s1, a = textslider(0.19:0.1:1.01, "Green")
s2, b = textslider(0.19:0.1:1.01, "Yellow")
s3, c = textslider(0.19:0.1:1.01, "Red")
vfield_dm = meshscatter(
    vec(Point3f0.(x, y, z)),
    rotation = vec(vectorfield_dm),
    marker = Pyramid(Point3f0(0), .5f0, .5f0),
...
Makie.vbox(vfield_dm, colorlegend(vfield_dm[end]))
...
params = [.5, 0.25, 0.5, 0.49, 0.05, 7.39]
@add_constraints rn1d begin
    Di + Da + Dm = 100.
end
ss = steady_states(rn1d,params)
sort!(ss, by = x -> x[1])
@show ss
@show target_low = ss[1][[1,2,4]]
@show target_high = ss[3][[1,2,4]]


# --------- ODE -----------
rn1d_ode = @ode_def begin
    dO = -O*d + b*(con -Di -Dm) + 2*ep*(con -Di -Dm) - O^2*a*Di
    dDi = -Di * gama + ep*(con -Di -Dm) + th*Dm + (-1/2)*O^2*a*Di
    dDm = Di * gama - th*Dm
end a ep b gama th d con

function first_hit_ss(sol_rd,tspan, eps)
    tss_set = []
    for ti = tspan[1] : 0.01 : tspan[2]
...
p = [.5, 0.25, 0.5, 0.49, 0.05, 7.39, 100.]
function Point_T_scaler(x,y,z; tspan = (0.0,1.5e2), p = p)
    ...
    t_ss = first_hit_ss(sol_rd,tspan,1e-1)[1]
    @show t_ss
    sss = sol_rd[end]
    @show sss

    attractor_low  = norm(sss .- target_low)  < norm(sss .- target_high) ? t_ss : 0
    attractor_high = norm(sss .- target_high) < norm(sss .- target_low)  ? t_ss : 0
    return attractor_low, attractor_high
end

Point_T_scaler(0.2, 9.25, 90.7)
Point_T_scaler(4.18, 3.5, 34.)

N = 30 ;r = LinRange(0., 100, N)
...
s1, v1 = textslider(sort(unique(Tc_low))[2]:0.1:sort(unique(Tc_low))[end], "slice")
density1 = Makie.volume(r,r,r,Tc_low, algorithm = :iso, isovalue = v1, color = :blue)
Makie.scatter!(density1, Point3f0[ target_low], marker = [ :circle],markersize =10)
Makie.vbox(s1, density1)

s2, v2 = textslider(sort(unique(Tc_high))[2]:0.1:sort(unique(Tc_high))[end], "slice")
density2 = Makie.volume(r,r,r,Tc_high, algorithm = :iso, isovalue = v2, color = :green, transparency = true)
Makie.scatter!(density2, Point3f0[target_high], marker = [:circle],markersize =10)
Makie.vbox(s2, density2)

Makie.vbox(s1, density1, s2, density2)
